...
int nalu_type(unsigned char* naluData, int naluSize)
{
    ...
    if(naluData[0]==0 && naluData[1]==0 && naluData[2]==0 &&(naluData[4]==0x61||naluData[4]==0x41))
    {
        index = NALU_P;
        printf("%s[%d]====NALU_P\n",__FUNCTION__,__LINE__);
    }
...
/*******************************************************************************
*@ Description    :开始fmp4编码
*@ Input          :<info>文件的输出信息
*@ Output         :
*@ Return         :成功：0 ；失败：-1
*@ attention      :如果刚开机，不要太快调用该接进行录像
*******************************************************************************/
int fmp4_record(fmp4_out_info_t* info)
{
    ...
    unsigned int frame_len = 0;//帧真实编码数据长度
	...
	//---debug 部分-------------------------------------------------------------------------
        ...
        int tmp_fd = open("/tmp/T20_IDR_NALU.bin",O_RDWR|O_CREAT|O_TRUNC,0777);
        if(tmp_fd < 0)
        {
    ...
    skip_len = sizeof (FRAME_HDR) + sizeof (IFRAME_INFO); 
    char * IDR_frame = frame_data + skip_len;
   ...
   while(1)//录制的时长用视频帧的帧头时间来进行计时比较准确
    {
         ...
         if (header->type == 0xFA)//0xFA-音频帧
        {
            ...
            if(Fmp4AEncode((unsigned char*)frame_data + skip_len,frame_len, A_FRAME_RATE,A_info->pts_msec)<0)
            {
        ...
        else if(header->type == 0xF8)   //0xF8-视频 IDR 帧 
        {
            V_count ++;
            printf("I frame ========\n");
            skip_len = sizeof (FRAME_HDR) + sizeof (IFRAME_INFO);
            ...
            if(Fmp4VEncode((unsigned char*)frame_data + skip_len,frame_len,V_FRAME_RATE,V_info->pts_msec)<0)
            {
